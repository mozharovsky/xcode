...
fn resolve_package(project: &XcodeProject, query: &str) -> Result<String, CliError> {
    ...
    let matches: Vec<_> = packages.iter().filter(|(_, _, loc)| loc.contains(query) || loc.ends_with(query)).collect();

    match matches.len() {
        0 => Err(CliError::new(ErrorCode::PackageNotFound, format!("No package matching '{}'", query))),
        1 => Ok(matches[0].0.clone()),
        _ => Err(CliError::new(ErrorCode::AmbiguousReference, format!("Multiple packages matched '{}'", query))),
    }
}
    ...
    match action {
        ...
        SpmAction::AddLocal { path, package_path, write, json } => {
            ...
            if json {
                ...
                println!("Added local package '{}' ({}){}", package_path, uuid, if write { "" } else { " (dry-run)" });
            }
            Ok(())
        }
